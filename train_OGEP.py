...
run_fn = {"PiNet": run_PiNet, "diffNet": run_DiffNet}


def parse_params():
    parser = argparse.ArgumentParser(description='Train on AbAg prediction.')
...
if __name__ == "__main__":
    ...
    pretrain_path = os.path.join(base_path, "pretrained_models/{}/{}/{}.pth".format(args.model,dataset_name, input_features))
    model_save_path = os.path.join(base_path, "saved_models/{}/{}/".format(args.model,dataset_name))
    ...
    if args.model == "PiNet":
        ...
        model = PiNet(feature_transform=params["feature_transform"], pdrop=params["pdrop"], id=feat_dim, geo=params["geo"],
                    ...
                    STN_decr_dims=params["STN_decr_dims"], seg_dims=params["seg_dims"])
        model.to(dtype).to(device)
    elif args.model == "DiffNet":
        feat_dim = dataset["train"][0]["ab_feats"].size(-1)
    ...
    for epoch in range(n_epoch):
        ...
        train_losses, train_metrics = run_epoch(model, dataset_loader["train"], params["losses_weight"],
                                                metrics=metrics, device=device, run_model=run_fn[args.model],
    ...
    torch.save({
        ...
        'scheduler_state_dict': scheduler.state_dict()}, os.path.join(model_save_path, "state_dict.pth"))
    print("...done!")
    ...
    test_metrics, curves_vals, test_metrics_res, curves_res_vals = test_model(model, dataset_loader["test"],
                                                                                ...
                                                                                device=device,
                                                                                run_model=run_fn[args.model], dtype=dtype,
                                                                                threshold=0.8,
                                                                                reduce_res="mean")
    df_dict = dict()
    for mol in ["ag", "ab"]:

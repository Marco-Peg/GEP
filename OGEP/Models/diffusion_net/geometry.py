...
def face_coords(verts, faces):
    coords = verts[faces.to(torch.int64)]
    return coords
...
def compute_operators(verts, faces, k_eig, normals=None, compute_grad=True):
    """
    ...
    if not compute_grad:
        return frames, massvec, L, evals, evecs
...
def get_all_operators(verts_list, faces_list, k_eig, op_cache_dir=None, normals=None):
    ...
    for num, i in tqdm(enumerate(inds)):
        ...
        if normals is None:
            outputs = get_operators(verts_list[i], torch.tensor([]) if faces_list is None else faces_list[i], k_eig,
                                    op_cache_dir)
        else:
            outputs = get_operators(verts_list[i], torch.tensor([]) if faces_list is None else faces_list[i], k_eig,
                                    op_cache_dir, normals=normals[i])
        frames[i] = outputs[0]
        massvec[i] = outputs[1]
        L[i] = outputs[2]
        evals[i] = outputs[3]

...
from OGEP.Models.models import PiNet
...
if __name__ == "__main__":
    ...
    pretrain_path = os.path.join(base_path, "pretrained_models/PiNet/{}/{}.pth".format(dataset_name, input_features))
    model_save_path = os.path.join(base_path, "saved_models/PiNet/{}/".format(dataset_name))
    ...
    for epoch in range(n_epoch):
        ...
        train_losses, train_metrics = run_epoch(model, dataset_loader["train"], params["losses_weight"],
                                                metrics=metrics, device=device, run_model=run_PiNet,
    ...
    if params["wandb_on"]:
        ...
        test_metrics, curves_vals, test_metrics_res, curves_res_vals = test_model(model, dataset_loader["test"],
                                                                                  ...
                                                                                  device=device,
                                                                                  run_model=run_PiNet, dtype=dtype,
                                                                                  threshold=0.8,
                                                                                  reduce_res="mean")
        df_dict = dict()
        for mol in ["ag", "ab"]:
